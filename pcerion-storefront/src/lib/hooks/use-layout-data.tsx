import { getProductsList, getCollectionsList, getCategoriesList, ProductCategoryWithChildren } from "@lib/data"
...
const fetchCategoryData = async (): Promise<LayoutCategory[]> => {
  let product_categories: ProductCategoryWithChildren[] = []
  let offset = 0
  let count = 1

  do {
    await getCategoriesList(offset)
    .then((res) => res)
    .then(({ product_categories: newCategories, count: newCount}) => {
      product_categories = [...product_categories, ...newCategories]
      count = newCount
  ...
  }while (product_categories.length < count)
  
  return product_categories.map((ct) => ({
    handle: ct.handle,
    name: ct.name,
  }))
} 
